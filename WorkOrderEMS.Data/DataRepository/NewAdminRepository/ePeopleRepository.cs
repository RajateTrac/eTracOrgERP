...
namespace WorkOrderEMS.Data
{
    public class ePeopleRepository
    {
        ...
        public List<spGetHiringGraph_Result> GetApplicantListData(long JobPostingId)
        {
            try
            {
                var HManager = objworkorderEMSEntities.spGetMyOpening(JobPostingId).FirstOrDefault();
                var data = objworkorderEMSEntities.spGetHiringGraph(HManager.JPS_HiringManagerID, JobPostingId).ToList();
                return data;
            }
            catch (Exception ex)
            {
