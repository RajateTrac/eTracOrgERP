...
namespace WorkOrderEMS.BusinessLogic
{
    public class ApplicantManager : IApplicantManager
    {
        ...
        public ServiceResponseModel<string> SignUpApplicant(eTracLoginModel obj)
        {
            ...
            try
            {
                if (obj.FName != null && obj.Email != null)
                {
                    ...
                    var isChanged = _db.spSetApplicantCreateLoginAccess(obj.LoginId, password, obj.FName, obj.MName, obj.LName, obj.Email, obj.Question, obj.Answer, obj.ApplicantPhoto).FirstOrDefault();
        ...
        public bool SendOffer(OfferModel model)
        {
            ...
            try
            {
                if (model != null)
                {
                    ...
                    var offerletterdetails = _db.spGetOfferLetter(model.ApplicantId).Select(x => new OfferModel()
                    {
                    ...
                    if (userData != null)
                    {
                        ...
                        objEmailHelper.HRPosition = model.HMPosition;
                        objEmailHelper.DueDate = model.DueDate;
                        objEmailHelper.OlLocationName = model.LocationName;
                        objEmailHelper.Currentdate = model.CurrentDate;
                        objEmailHelper.OLTitle = model.Title;
                        objEmailHelper.PhoneNo = model.PhoneNumber;
                        objEmailHelper.TimeAttempted = DateTime.UtcNow.ToMobileClientTimeZone(objTemplateModel.TimeZoneName, objTemplateModel.TimeZoneOffset, objTemplateModel.IsTimeZoneinDaylight, false).ToString();
                        IsSent = objEmailHelper.SendEmailWithTemplate();
                        if (IsSent == true)
                        {
        ...
        public bool SaveApplicantData(CommonApplicantModel Obj)
        {
            ...
            try
            {
                ...
                using (var context = new workorderEMSEntities())
                {
                    ...
                    Flag = true;
                    var saveStatus = _db.spSetApplicantStatus(Obj.ApplicantId, "A", "Y");
                    var saveSacore = _db.spSetApplicantScore();
                    var update = _db.Applicants.Where(x => x.APT_ApplicantId == Obj.ApplicantId).FirstOrDefault();
                    var updateLoginAccess = _db.ApplicantLoginAccesses.Join(_db.Applicants, q => q.ALA_UserId, u => u.APT_ALA_UserId, (q, u) => new { q, u }).
                                           Where(x => x.u.APT_ApplicantId == Obj.ApplicantId).FirstOrDefault();
                    if (update != null && Obj.ApplicantAdditionalInfo.Count() > 0 && Obj.ApplicantAdditionalInfo[0].AAI_AvailableDate != null && updateLoginAccess != null)
                    {
                        var _repo = new ApplicantStatusRepository();
                        var _repoLogin = new ApplicantLoginAccessRepository();
                        update.APT_DateOfJoining = Obj.ApplicantAdditionalInfo[0].AAI_AvailableDate;
                        updateLoginAccess.q.ALA_Photo = Obj.ApplicantPhoto;
                        _repo.Update(update); _repoLogin.Update(updateLoginAccess.q);
                        _repo.SaveChanges(); _repoLogin.SaveChanges();
                    }
        ...
        public ContactListModel GetContactListByApplicantId(long ApplicantId)
        {
            ...
            try
            {
                var getDetails = _db.spGetApplicantContactInfo(ApplicantId).Where(x => x.ACI_PrefredContactMethod != null).Select(x => new ContactModel()
                {
                    ContactId = x.ACI_Id,
        ...
        /// Created For : To send mail to applicant after hired
        /// </summary>
        /// <param name="ApplicantId"></param>
        /// <returns></returns>
        public bool SendUserNameAndPassword(long ApplicantId)
        {
            bool isSend = false;
            try
            {
                if (ApplicantId > 0)
                {
                    var getEMPData = _db.spGetApplicantAllDetails(ApplicantId).FirstOrDefault();
                    var getLocationCode = _db.LocationMasters.Where(x => x.LocationId == getEMPData.LocationId).FirstOrDefault().Address2.Substring(0, 3).ToUpper();
                    var getLastEmp_Id = _db.Employees.OrderByDescending(x => x.EMP_Id).FirstOrDefault().EMP_Id;
                    if (getEMPData != null && getLocationCode != null && getLastEmp_Id > 0)
                    {
                        ...
                        var employeeID = getLocationCode + "000" + value;
                        var saveEployee = _db.spSetEmployee("I", null, employeeID, ApplicantId, getEMPData.API_FirstName, getEMPData.API_MidName,
                                                                  getEMPData.API_LastName, getEMPData.ACI_eMail, getEMPData.ACI_PhoneNo, null, null, null, getEMPData.ALA_Photo, null, 9,
                                                                  getEMPData.JBT_Id, getEMPData.HiringManagerEmployeeId, getEMPData.APT_DateOfJoining, getEMPData.LocationId,
                        ...
                        if (ApplicantId > 0)
                        {
                            ...
                            if (getEMPData != null)
                            {
                                ...
                                objEmailHelper.MailType = "HIRED";
                                objEmailHelper.Subject = "eTrac : You are hired for position";
            ...
            {
                isSend = false;
                Exception_B.Exception_B.exceptionHandel_Runtime(ex, "public bool SendUserNameAndPassword(long ApplicantId)", "Exception While sending mail to applicant.", ApplicantId);
                throw;
            }
            return isSend;
        }
        ...
        public BackgroundCheckForm GetApplicantByApplicantId(long ApplicantId)
        {
            ...
            try
            {
                ...
                new Models.ApplicantPersonalInfoBackground()
                {
        ...
        public bool SendApplicantInfoForBackgrounddCheck(BackgroundCheckForm model)
        {
            ...
            try
            {
                if (model != null && model.ApplicantPersonalInfo.API_APT_ApplicantId > 0)
                {
                    ...
                    var getQBKID = _db.CompanyQBKs.Where(x => x.QBK_CMP_Id == company.CMP_Id).FirstOrDefault();
                    ...
                    var saveNormalPO = _db.spSetPODetail("I", poNum, getHRDetail.jp.JPS_LocationId,
                                                                         POType, company.CMP_Id, null,
                                                                          null, null, null, model.UserId, model.UserId, "Y", null
                                                                          , null, null, getQBKID.QBK_RefId, null);
                    if (getHRDetail != null)
                    {
        ...
        /// <param name="ApplicantId"></param>
        /// <returns></returns>
        public I9FormModel GetI9FormData(long ApplicantId, long UserId, FormNameStatus obj)
        {
            ...
            try
            {
                if (ApplicantId > 0 && UserId > 0)
                {
                    ...
                    if (_modelI9 == null)
                    {
                        var model = new I9FormModel();
                        var getApplicant = GetApplicantAllDetails(ApplicantId);
                        model.I9F_Sec1_LastName = getApplicant.LastName;
                        model.I9F_Sec1_FirstName = getApplicant.FirstName;
                        model.I9F_Sec1_MiddleInitiaL = getApplicant.MiddleName;
                        model.I9F_Sec1_City = getApplicant.City;
                        model.I9F_Sec1_CitizenOfUS = getApplicant.Cityzenship;
                        model.I9F_Sec1_Email = getApplicant.Email;
                        model.I9F_Sec1_State = getApplicant.State;
                        model.I9F_Sec1_ZipCode = getApplicant.Zip;
                        model.I9F_Sec1_Address = getApplicant.Address;
                        model.I9F_Sec3_A_FirstName = getApplicant.FirstName;
                        model.I9F_Sec3_A_LastName = getApplicant.LastName;
                        model.I9F_Sec3_A_MiddleInitial = getApplicant.MiddleName;
                        model.I9F_Sec3_B_DateOfReHire = getApplicant.AvailableDate;
                        model.I9F_Sec1_SSN = getApplicant.SocialSecurityNumber;
            ...
            try
            {
                ...
                if (UserId > 0 && ApplicantId > 0 && model != null && model.I9F_Id == null)
                {
                    ...
                    var save = _db.spSetI9Form(action, model.I9F_Id, model.I9F_EMP_EmployeeId, model.I9F_Sec1_SSN, model.I9F_Sec1_CitizenOfUS, model.I9F_Sec1_NonCitizenOfUS,
                               ...
                               model.I9F_Sec3_C_DateOfEmployerOrAuthorizedSign, model.I9F_Sec3_C_NameOfEmployerOrAuthorized, model.I9F_Date, "Y", "11111");//,getOutputData.case_number);
        ...
        /// to get isexempt from employee table
        /// </summary>
        /// <param name="EmployeeId"></param>
        /// <returns></returns>
        public RateOfPayModel GetIsExempt(string EmployeeId)
        {
            var model = new RateOfPayModel();
            var getIsexempt = _db.Employees.Where(x => x.EMP_EmployeeID == EmployeeId).FirstOrDefault();
            model.IsExempt = getIsexempt.EMP_IsExempt;
            return model;
        }
        ...
        public bool SaveOriantation(OriantationModel model)
        {
            ...
            try
            {
                ...
                var statusSaved = SetApplicantStatus(model.OTN_ApplicantId, ApplicantStatus.Oriantation, ApplicantIsActiveStatus.Sent);
                isSaved = true;
            }
            catch (Exception ex)
            {
        ...
        public bool InterviewCompleted(long ApplicantId)
        {
            return _db.spSetApplicantStatus(ApplicantId, ApplicantStatus.Interview, "C") > 0 ? true : false;
        }
        ...
        public EmployeeVIewModel GetApplicantAllDetails(long ApplicantId)
        {
            return _db.spGetApplicantAllDetails(ApplicantId).Select(x => new EmployeeVIewModel()
            {
                ...
                Email = x.ACI_eMail,
                Phone = x.ACI_PhoneNo,
                AvailableDate = x.APT_DateOfJoining,
        ...
        /// Created For : To get applicant counts for graph
        /// </summary>
        /// <returns></returns>
        public ApplicantCountDetails GetApplicantCount()
        {
            var counts = new ApplicantCountDetails();
            try
            {
                var getcount = _db.Applicants.ToList();
                counts.TotalApplicant = getcount.Count();
                counts.HiredApplicant = getcount.Where(x => x.APT_Status == "Hired").Count();

            }
            catch (Exception ex)
            {
                Exception_B.Exception_B.exceptionHandel_Runtime(ex, "public ApplicantCountDetails GetApplicantCount()", "Exception While getting the applicant count.", null);
            }
            return counts;
        }
        ...
        /// <returns></returns>
        public bool CancelInterviewManager(string InterviewerID, long ApplicantId, string Comment)
        {
            bool isSaved = false;
            try
            {
                ...
                {
                    var save = _db.spSetScheduleInterviewCancelled_Reset(ApplicantId, InterviewerID, Comment, null);
                    isSaved = true;
                }
                else
                {
            ...
            {
                Exception_B.Exception_B.exceptionHandel_Runtime(ex, "public bool CancelInterviewManager(string InterviewerID, long ApplicantId, string Comment)", "Exception While cancel applicant interview.", null);
            }
            return isSaved;
        }
        public bool SetApplicantStatus(long ApplicantId, string Status, string IsActive)
        {
            return _db.spSetApplicantStatus(ApplicantId, Status, IsActive) > 0 ? true : false;
        }
        public bool SendMailToEmployeeOriantationDone(long ApplicantId, string EmployeeId, string IsActive)
        {
            bool isSaved = false;
            ...
            try
            {
                if (ApplicantId > 0)
                {
                    if (IsActive == "Y")
                    {
                        var getData = _db.Employees.Where(x => x.EMP_API_ApplicantId == ApplicantId).FirstOrDefault();
                        Emp_Id = getData.EMP_EmployeeID;
                        using (var context = new workorderEMSEntities())
                        {
                            var userData = userRepo.GetSingleOrDefault(x => x.EmployeeID == getData.EMP_EmployeeID && x.IsDeleted == false);
                            userData.UserType = Convert.ToInt64(UserType.Manager);
                    ...
                    if (getEMPData != null)
                    {
                        ...
                        //By Default password will be this will change it if requirment changes
                        objEmailHelper.Password = "Elite@123";
                        objEmailHelper.UserName = Emp_Id;
                        objEmailHelper.JobTitle = getEMPData.JBT_JobTitle;
                        //objEmailHelper.AcceptAssessmentLink = HostingPrefix + "api/ServiceApi/GetAssessmentList?ApplicantId="+ ApplicantId+"&";
                        objEmailHelper.AcceptAssessmentLink = HostingPrefix;
                        objEmailHelper.MailType = "ORIANTATIONDONE";
                        objEmailHelper.Subject = "eTrac : Your Oriantation has been" + " " + IsActive == "Y" ? "Done" : "Fail";
                        objEmailHelper.SentBy = Convert.ToInt64(getEMPData.UserId);
            ...
            {
                Exception_B.Exception_B.exceptionHandel_Runtime(ex, "public bool SendMailToEmployeeOriantationDone(long ApplicantId)", "Exception While sending mail.", ApplicantId);
            }
            return isSaved;
        }
        ...
        /// <summary>
        /// Created By  :Ashwajit Bansod
        /// Created For : To get file of onboarding for single pdf download.
        /// </summary>
        /// <param name="fileName"></param>
        /// <param name="EmployeeID"></param>
        /// <returns></returns>
        public string GetFilesData(string fileName, string EmployeeID)
        {
            try
            {
                if (fileName != null)
                {
                    var geData = _db.FileUploads.Where(x => x.FLU_EMP_EmployeeId == EmployeeID && x.FLU_FileName == fileName).FirstOrDefault();
                    return geData.FLU_FileAttached;
                }
                else
                    return null;
            }
            ...
            {
                Exception_B.Exception_B.exceptionHandel_Runtime(ex, "public string GetFilesData(string fileName)", "Exception While getting onboarding file of applicant.", null);
                throw;
            }
        }
        /// <summary>
        ...
        /// <param name="applicantId"></param>
        /// <returns></returns>
        public Employee GetEmployeeData(long applicantId)
        {
            try
            {
                if (applicantId > 0)
        ...
        public ApplicantFunFactModel GetFunFacts(string EmpId)
        {
            var data = new ApplicantFunFactModel();
            try
            {
                var Id = Convert.ToInt64(InterviewQuestionsId.MasterId);
                if (EmpId != null)
                {
                    var getData = _db.InterviewAnswers.Where(x => x.INA_EMP_EmployeeId == EmpId && x.INA_IQC_Id == Id).ToList();
                    if (getData.Count() > 0)
                    {

                        if (getData.Count() == 1)
                        {
                            data.Employee_Id = getData[0].INA_EMP_EmployeeId;
                            data.Answer_Que1 = getData[0].INA_Comments;
                        }
                        if (getData.Count() == 2)
                        {
                            data.Answer_Que1 = getData[0].INA_Comments;
                            data.Answer_Que2 = getData[1].INA_Comments;
                        }
                        if (getData.Count() == 3)
                        {
                            data.Answer_Que1 = getData[0].INA_Comments;
                            data.Answer_Que2 = getData[1].INA_Comments;
                            data.Answer_Que3 = getData[2].INA_Comments;
                        }
                        else
                        {
                            data.Answer_Que1 = getData[0].INA_Comments;
                            data.Answer_Que2 = getData[1].INA_Comments;
                            data.Answer_Que3 = getData[2].INA_Comments;
                            data.Answer_Que4 = getData[3].INA_Comments;
                        }
                    }
            ...
            {
                Exception_B.Exception_B.exceptionHandel_Runtime(ex, "public ApplicantFunFactModel GetFunFacts(string EmpId)", "Exception While getting data of funfacts.", EmpId);
                throw;
            }
        }
        /// <summary>
        ...
        /// Created For : To update funfacts
        /// </summary>
        /// <param name="obj"></param>
        /// <returns></returns>
        public bool UpdateFunFacts(ApplicantFunFactModel obj)
        {
            bool isSaved = false;
            int save = 0;
            int i = 0;
                ...
                if (obj != null)
                {
                    ...
                    for (i = 0; i < getQuestionsIdForFunFacts.Count(); i++)
                    {
                        ...
                        if (getQuestionsIdForFunFacts[i].IQC_Id == Convert.ToInt64(InterviewQuestionsId.QuestionId_45))
                            ...
                            Answer = obj.Answer_Que4;
                        save = _db.spSetInterviewAnswerUpdate(getQuestionsIdForFunFacts[i].IQC_Id, obj.Applicant_Id, obj.Employee_Id, null, Answer);
                    }
                    isSaved = save > 0 ? true : false;
                    return isSaved;
                }
