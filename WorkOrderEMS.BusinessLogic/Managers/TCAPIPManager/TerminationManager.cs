...
namespace WorkOrderEMS.BusinessLogic
{
    public class TerminationManager : ITerminationManager
    {
        ...
        public TerminationManager(IDepartment _IDepartment)
        {
        ...
        /// <returns></returns>
        public bool SaveTerminationFormDetails(TerminationModel Obj, long LoginEmployeeId)
        {
            ...
            try
            {
                if (Obj != null)
                {
                    _db.spSetTerminationForm("I", null, Obj.EmpId, Obj.ManagerId, Obj.Termination_Date, Obj.Last_Day_Worked, Obj.Reason_For_Leaving, Obj.detailed_Expalnation, Obj.Re_Hire, Obj.HR_Decision, Obj.IsSeverence, Obj.LengthOfSeverence, Obj.SeverenceApproval, Obj.WitnessedId, "");

                    var getdatailsHR = _IDepartment.GetDepartmentEmployeeList(WorkOrderEMS.Helper.Department.HR);//changed by rajat
                    if (getdatailsHR.Count() == 0)
                    {
                        foreach (var item in getdatailsHR)
                        {
                            objNotification.Message = DarMessage.TerminationRequestForEmployee(Obj.Emp_Name, Obj.Manager_Name);
                            objNotification.Module = ModuleSubModule.ePeople;
                            objNotification.SubModule = ModuleSubModule.TerminationRequest;
                            objNotification.SubModuleId1 = Obj.EmpId;
                            objNotification.CreatedByUser = Convert.ToString(LoginEmployeeId);
        ...
        /// <param name="Tstatus"></param>
        /// <returns></returns>
        public List<TerminationListModel> TerminationlistFinalizedetails(string man_id, DateTime? ToDate, DateTime? FromDate)
        {
            TerminationListModel obj = new TerminationListModel();
            try
            {
        ...
        /// <param name="Tstatus"></param>
        /// <returns></returns>
        public List<TerminationListModel> TerminationlistPendingdetails(string man_id, DateTime? ToDate, DateTime? FromDate)
        {
            TerminationListModel obj = new TerminationListModel();
            try
            {
        ...
        public bool SaveNotificationForHrApproval(string EmpId, string LoginEmployeeId)
        {
            ...
            try
            {
                if (EmpId != null)
                {
                    ...
                    objNotification.SubModuleId1 = EmployeeDetails.EMP_EmployeeID;
        ...
        public void SendEmailToPayrollForAllAssetsReturned(List<EmployeeAssetDetails> obj, string EmpId, string LoginEmployeeId)
        {
            #region Email
            var objEmailLogRepository = new EmailLogRepository();
            var objEmailReturn = new List<EmailToManagerModel>();
            ...
            var objTemplateModel = new TemplateModel();
            //var userData = _db.ApplicantLoginAccesses.Where(x => x.ALA_LoginId == Obj.ManagerId).FirstOrDefault();
            var userData = _db.Employees.Where(x => x.EMP_EmployeeID == EmpId).FirstOrDefault();
            if (userData != null)
            {
                bool IsSent = false;
                var objEmailHelper = new EmailHelper();
                var EmpName = userData.EMP_FirstName + " " + userData.EMP_LastName;
                objEmailHelper.Name = EmpName;
                objEmailHelper.ManagerName = LoginEmployeeId;
                objEmailHelper.EmployeeId = userData.EMP_EmployeeID;

                objEmailHelper.emailid = "rajat99938@gmail.com";


                objEmailHelper.MailType = "AllASSETSRETURNED";
                ...
                if (IsSent == true)
                {
                    ...
                    try
                    {
                        objEmailog.CreatedBy = Convert.ToInt64(LoginEmployeeId);
                        objEmailog.CreatedDate = DateTime.UtcNow;
                        objEmailog.DeletedBy = null;
                        objEmailog.DeletedOn = null;
                        objEmailog.ModifiedBy = null;
                        objEmailog.ModifiedOn = null;
                        objEmailog.SentBy = Convert.ToInt64(LoginEmployeeId);
                        objEmailog.SentEmail = "rajat99938@gmail.com";
                        objEmailog.Subject = objEmailHelper.Subject;
                        objListEmailog.Add(objEmailog);
                        //userData.ALA_eMailId
        ...
        public void SendEmailToPayrollForLostAssets(List<string> LostAssets, string EmpId, string LoginEmployeeId)
        {
            #region Email
            var objEmailLogRepository = new EmailLogRepository();
            var objEmailReturn = new List<EmailToManagerModel>();
            ...
            if (userData != null)
            {
                ...
                objEmailHelper.ManagerName = LoginEmployeeId;
                objEmailHelper.EmployeeId = userData.EMP_EmployeeID;
                foreach (string item in LostAssets)
                {
                    objEmailHelper.AssetLost = item;
                }

                objEmailHelper.emailid = "rajat99938@gmail.com";


                objEmailHelper.MailType = "ASSETSLOST";
        ...
        public void SaveAssetStatusReturnDate(string EmployeeId, string AssetName)
        {
            var details = _db.AssetAllocations.Where(x => x.ATA_EMP_EmployeeId == EmployeeId && x.ATA_AssetName == AssetName).FirstOrDefault();
            _db.spSetAssetAllocation("U", details.ATA_Id, details.ATA_EMP_EmployeeId, details.ATA_Type, details.ATA_AssetName, details.ATA_AssetDescription, details.ATA_Make, details.ATA_Model, details.ATA_SerialNumber, details.ATA_Login, details.ATA_Password, details.ATA_AssignDate, details.ATA_ReturnAcceptBy, details.ATA_ReturnDate, "R", "");
        }
    }
