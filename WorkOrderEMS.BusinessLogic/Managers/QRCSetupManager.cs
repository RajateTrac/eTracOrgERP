...
namespace WorkOrderEMS.BusinessLogic.Managers
{
    public class QRCSetupManager : IQRCSetup
    {
        ...
        public QRCModel GetGlobalCodeForCategories()
        {
            ...
            ObjQRCModel.QRCTypeList = ObjGlobalCodeList.Where(g => g.Category == "QRCTYPE" && g.IsDeleted != true).ToList();
        ...
        */
        ...
        public QRCModel GetQrcById(long qrcId, long locationid)
        {
            try
            {
                ...
                ObjQRCModel.CreatedOn = ObjQRCMaster.CreatedDate.ToClientTimeZone(true);
                ObjQRCModel.CreatedBy = ObjQRCMaster.CreatedBy;
                ObjQRCModel.ModifiedBy = ObjQRCMaster.ModifiedBy;
                ...
                if (ObjQRCMaster.QRCTYPE == Convert.ToInt64(37) && ObjQRCMaster.QRCTypeDetails != null)
                {
                    ...
                    ObjReturnTrash = GenericDataContractSerializer<ServiceQrcTrashcanModel>.DeserializeXml(ObjQRCMaster.QRCTypeDetails);
                    var TrashLevel = TrashLevelValue.TwentyFive;
                    if (ObjReturnTrash.Routine.TrashLevels == 157 && ObjReturnTrash.TrashRemoval.TrashLevels == 157 && ObjReturnTrash.TrashRemoval.IsTrashRemovalComplete == true) //If Trash Removal is full 157 is for full
                    {
                        ObjQRCModel.RoutineTrashLevels = Convert.ToInt32(TrashLevel);
                        ObjQRCModel.RemovalTrashLevels = Convert.ToInt32(TrashLevel);
                    }
                    //else if(ObjReturnTrash.Routine.TrashLevels == 157 && ObjReturnTrash.TrashRemoval.TrashLevels != 157)
                    //{
                    //    ObjQRCModel.RoutineTrashLevels = Convert.ToInt32(TrashLevel);
                    //    ObjQRCModel.RemovalTrashLevels = ObjReturnTrash.TrashRemoval.TrashLevels;
                    //}
                    else if (ObjReturnTrash.Routine.TrashLevels != 157 && ObjReturnTrash.TrashRemoval.TrashLevels == 157 && ObjReturnTrash.TrashRemoval.IsTrashRemovalComplete == true)
                    {
                      ObjQRCModel.RoutineTrashLevels = Convert.ToInt32(TrashLevel);
                      ObjQRCModel.RemovalTrashLevels = Convert.ToInt32(TrashLevel);
                    }
                    else if(ObjReturnTrash.TrashRemoval.TrashLevels >= 160 && ObjReturnTrash.TrashRemoval.IsTrashRemovalComplete == true)
                    {
                        ObjQRCModel.RoutineTrashLevels = Convert.ToInt32(TrashLevel);
                        ObjQRCModel.RemovalTrashLevels = Convert.ToInt32(TrashLevel);
                    }
                    else if (ObjReturnTrash.Routine.TrashLevels != 157 && ObjReturnTrash.TrashRemoval.TrashLevels != 157 && ObjReturnTrash.TrashRemoval.IsTrashRemovalComplete == false)
                    {
                            ObjQRCModel.RoutineTrashLevels = ObjReturnTrash.Routine.TrashLevels;
                            ObjQRCModel.RemovalTrashLevels = ObjReturnTrash.TrashRemoval.TrashLevels;
                    }
        ...
        public ServiceResponseModel<QRCModel> GetQRCDetailsByID(ServiceQrcModel ObjServiceQRCModel)
        {
            ...
            var ObjServiceResponseModel = new ServiceResponseModel<QRCModel>();
            try
            {
                if (ObjServiceQRCModel != null && ObjServiceQRCModel.QrcId > 0 && ObjServiceQRCModel.LocationId > 0)
                {
                    ...
                    if (authuser != null && authuser.UserId > 0)
                    {
                        ...
                        if (result != null)
                        {
                            ...
                            if (result.CheckOutStatus == false || result.CheckOutStatus == null)
                            {
                            ...
                            {
                                if (result.CheckOutStatus == true && result != null)
                                {
                                    if (authuser.UserId == result.ModifiedBy)
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        objNotification.IsCheckOut = true;
                                        var saveDataForNotification = _ICommonMethod.SaveNotificationDetail(objNotification);
                                        if (authuser.ManagerForUser > 0)
                                        {
                                            var managerDetail = ObjUserRepository.GetSingleOrDefault(x => x.UserId == authuser.ManagerForUser && x.IsDeleted == false);
                                            EmailHelper objEmailHelper = new EmailHelper();
                                            objEmailHelper.MailType = "CHECKINCHECKOUT";
                                            objEmailHelper.LocationName = result.LocationName;
                                            objEmailHelper.QrcName = result.QRCName;
                                            objEmailHelper.QrCodeId = result.QRCodeID;
                                            objEmailHelper.IsWorkable = false;
                                            objEmailHelper.Message = CommonMessage.CheckInOutMessageForManager(authuser.FirstName, authuser.LastName, result.QRCodeID, result.UserName);
                                            if (managerDetail.DeviceId != null)
                                            {
                                                PushNotificationFCM.FCMAndroid("The QRC"+ result.QRCName+ " "+"is checked out by"+ " " +result.UserName, managerDetail.DeviceId, objEmailHelper);
                                            }
                                        }
                                        ObjServiceResponseModel.Response = Convert.ToInt32(ServiceResponse.SuccessResponse, CultureInfo.CurrentCulture);
                                        ObjServiceResponseModel.Message = CommonMessage.CheckInOutStatus();
                                        ObjServiceResponseModel.Data = null;
                                    }
                                }
                            }                            
                        }
                        if(ObjServiceQRCModel.Lat != null && ObjServiceQRCModel.Long != null)
                        {
                            authuser.Latitute = ObjServiceQRCModel.Lat;
                            authuser.Longitude = ObjServiceQRCModel.Long;
        ...
        public ServiceResponseModel<string> SaveQrcElevatorRequestDetails(ServiceQrcElevatorModel obj)
        {
            ...
            try
            {
                if (obj != null)
                {
                    ...
                    ObjServiceResponseModel.Response = (result != null) ? Convert.ToInt64(result.Data.Response, CultureInfo.InvariantCulture) : Convert.ToInt32(ServiceResponse.SuccessResponse, CultureInfo.CurrentCulture);
        ...
        public ServiceResponseModel<string> SaveQrcPhoneSystemsRequestDetails(ServiceQrcPhoneSystemModel obj)
        {
            ...
            try
            {
                if (obj != null)
                {
                    ...
                    ObjServiceResponseModel.Response = (result != null) ? Convert.ToInt64(result.Data.Response, CultureInfo.CurrentCulture) : Convert.ToInt32(ServiceResponse.SuccessResponse, CultureInfo.CurrentCulture);
                    ObjServiceResponseModel.Message = (result != null) ? (result.Data.ResponseMessage).ToString() : CommonMessage.NoRecordMessage();//CommonMessage.MessageLogout();
                }
                else
                {
        ...
        public ServiceResponseModel<string> SaveQrcVehicleRequestDetails(ServiceQrcVehicleModel obj)
        {
            ...
            try
            {
                if (obj.ServiceAuthKey != null)
                {
                    ...
                    {
                        objEmailReturn = objEmailLogRepository.SendEmailToManagerForItemMissingQRC(obj.LocationId, obj.UserId);
                        var user = _db.UserRegistrations.Where(x => x.ServiceAuthKey == obj.ServiceAuthKey && x.IsEmailVerify == true && x.IsDeleted == false).FirstOrDefault();
                        if (user != null)
                        {
                            userData = _db.UserRegistrations.Where(x => x.UserId == user.ManagerForUser && x.IsDeleted == false && x.IsEmailVerify == true).Select(a => new UserModel()
                            {
                                UserId = a.UserId,
                       ...
                       //if(userData != null)
                        {
                            ...
                            string.IsNullOrEmpty(obj.CheckingOut.ItemDescription) == false)
                            {
                                EmailHelper objEmailHelper = new EmailHelper();
                                objEmailHelper.emailid = item.ManagerEmail;
                                objEmailHelper.ManagerName = item.UserName;
                            ...
                            if (obj.CheckingOut.IsDamage == true)
                            {
                                ...
                                PushNotificationFCM.FCMAndroid(message, item.DeviceId, objEmailHelper);
                                //QRCVehicleDamageNotification(obj,objEmailHelper, message);
                                if (IsSent == true)
                                {
                                    objEmailog = new EmailLog();
                                    try
        ...
        //private void QRCVehicleDamageNotification(ServiceQrcVehicleModel obj, EmailHelper objEmailHelper,string message)
        //{
        //    workorderEMSEntities _db = new
        //    try
        //    {
        //        if (obj != null && objEmailHelper != null && message != null)
        //        {
        //            if (obj.UserId > 0)
        //            {
        //                var userDate = 
