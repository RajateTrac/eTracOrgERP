    ...
    public class LoginManager : ILogin
    {
            ...
            try
            {
                ...
                if (authuser != null && authuser.UserId > 0)
                {
                    ...
                    if ((loginViewModel.DeviceType > 0) || !string.IsNullOrEmpty(loginViewModel.DeviceId))
                    {
                        ...
                        if (loginViewModel.Lat != null)
                        {
                            authuser.Latitute = loginViewModel.Lat;
                    ...
                    //For to get latitute and longitude to mobile user
                    loginViewModel.Lat = loginViewModel.Lat;
                    loginViewModel.Long = loginViewModel.Long;
                    loginViewModel.UserType = authuser.UserType;
        ...
        public bool ChangePassword(eTracLoginModel eTracLogin, out string message)
        {
            ...
            try
            {
                ...
                var UserDetail = ObjUserRepository.GetSingleOrDefault(u => u.UserId == eTracLogin.UserId && u.Password == mypassword);
        ...
        /// <param name="ServiceAuthKey"></param>
        /// <returns></returns>
        public List<ServiceWorkAssignmentModel> GetContinuousTaskListByEmployeeId(string serviceAuthKey, long userId, long locationId, string TimeZoneName)
        {
            WorkRequestAssignmentRepository _WorkRequestAssignmentRepository = new WorkRequestAssignmentRepository();
            try
            {
                                                          ...
                                                          locationId).Select(t => new ServiceWorkAssignmentModel()
                                                             {
                                                                 ...
                                                                 //WorkRequestImage = t.AssignedWorkOrderImage == "" ? ConfigurationManager.AppSettings["hostingPrefix"].ToString() + "Content/Images/WorkRequest/no-asset-pic.png" : ConfigurationManager.AppSettings["hostingPrefix"].ToString() + "Content/Images/WorkRequest/" + t.AssignedWorkOrderImage,
                                                                 ...
                                                                 StartTime = t.ConStartTime.Value.GetClientDateTimeForMobileNow(TimeZoneName).ToString("hh:mm tt"),
                                                                 ...
                                                                 WorkRequestStatus = t.WorkRequestStatus,
                                                                 WorkRequestStatusName = t.WorkRequestStatusName
                                                             }).OrderByDescending(x => x.WorkRequestAssignmentID).ToList();
                // List<ServiceWorkAssignmentModel> tasklist = new List<ServiceWorkAssignmentModel>();
                return tasklist;
            }
            catch (Exception ex)
