...
namespace WorkOrderEMS.BusinessLogic.Interfaces
{
    public interface ITerminationManager
    {
        ...
        bool SaveTerminationFormDetails(TerminationModel Obj, long LoginId);

        List<TerminationListModel> TerminationlistTeamDetails(string LoginMangerId, DateTime? ToDate, DateTime? FromDate);

        List<TerminationListModel> TerminationlistFinalizedetails(string man_id, DateTime? ToDate, DateTime? FromDate);

        List<TerminationListModel> TerminationlistPendingdetails(string man_id, DateTime? ToDate, DateTime? FromDate);
        ...
        void SendEmailToPayrollForAllAssetsReturned(List<EmployeeAssetDetails> obj, string EmpId, string LoginEmployeeId);

        void SendEmailToPayrollForLostAssets(List<string> LostAssets, string EmpId, string LoginEmployeeId);
        void SaveAssetStatusReturnDate(string EmployeeId, string AssetName);
