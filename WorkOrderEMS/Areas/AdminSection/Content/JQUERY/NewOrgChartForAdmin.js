...
OrgChart.prototype.load = function (t) {
            ...
            (a = f[u]).addEventListener("mouseover", function () {
                ...
                if (void 0 === l[r].onClick)
                    if ("add" === r) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '../AdminSection/AdminDashboard/GetJobTitleById?CSVChartId=' + i.pid, //'@Url.Action("SaveVCS", "AdminDashboard", new { area = "AdminSection" })',
                            success: function (Data) {
                                debugger
                                $(".removeContent input[type='text']").val("");
                                if (Data.length > 0) {
                        ...
                        $.ajax({
                            ...
                            success: function (Data) {
                                ...
                                if (Data != null) {
                                    ...
                                    $("#VCSId").val(Data.Id);
                                    $("#routeDiv").html("");
                                   // $('#EmploymentStatus option[value="' + Data.EmploymentStatus + '"]').attr("selected", true);
                                    $("#EmploymentClassification").get(0).selectedIndex = Data.EmploymentClassification == "Y" ? 1 : 2;
                                    $("#EmploymentStatus").get(0).selectedIndex = Data.EmploymentStatus == "FT" ? 1 : Data.EmploymentStatus == "PT"?2:3;
                                    //$("#EmploymentStatus").val(Data.EmploymentStatus);
                                    //$("#EmploymentClassification").val(Data.EmploymentClassification);
                                    //$('#EmploymentClassification option[value="' + Data.EmploymentClassification + '"]').attr("selected", true);
                                    $("#RateOfPay").val(Data.RateOfPay);
                                ...
                                $("#SaveVSC").val("Update");
                                //$("#RolesAndResponsibility").val(roles);
                                tinymce.activeEditor.setContent(roles);
    ...
    this._draw(!1, OrgChart.action.exporting, t, function (t) {
        ...
        if (r.innerHTML = t, "svg" == a.ext)
            if (o) o(a, r.innerHTML);
                ...
                if (console.log(JSON.stringify(e)), !1 === OrgChart.events.publish("exportstart", [n, e])) return !1;
    ...
    for (c = 0; c < n.length; c++) {
        ...
        e == OrgChart.action.exporting && null != r.id && (l = [o[r.id]]), y ? (v.state = y.state, v.state == OrgChart.COLLAPSE && m.push(v.id), OrgChart.server._setNodeGroupState(v, y.groupState, t.orientation)) : OrgChart.server._setNodeGroupState(v, OrgChart.EXPAND, t.orientation), OrgChart.server._initStates(v, o, e, t, r, m), OrgChart.server._initGroupStates(v, o, e, t, r)
    }
    if (null != t.roots) {
        l = [];
    ...
    for (c = 0; c < s.length; c++) {
        ...
        if (H) {
            ...
            (o[_.id] = _).isAssistant = !0, (y = i[_.id]) && (o[_.id].state = y.state, o[_.id].state == OrgChart.COLLAPSE && m.push(_.id)), OrgChart.server._initStates(_, o, e, t, r, m);
            var k = [];
            for (b = H.children.length - 1; 0 <= b; b--) {
                (z = H.children[b]).isAssistant ? (z.parent = null, H.children.splice(b, 1), H.childrenIds.splice(b, 1), k.splice(0, 0, z.id)) : (z.pid = _.id, (z.parent = _).children.push(z), _.childrenIds.push(z.id), H.children.splice(b, 1))
            }
            if (k.length % 2) {
    ...
    if (e == OrgChart.action.centerNode) {
        var P = o[r.id];
        P || (P = o[h[r.id]], OrgChart.server._setNodeGroupState(P, OrgChart.EXPAND, t.orientation)), P.state = OrgChart.EXPAND, -1 != (D = m.indexOf(P.id)) && m.splice(D, 1);
        for (var H = OrgChart.server._getRealParent(P), U = P.id; null != H;) {
            if (-1 != (D = m.indexOf(H.id)) && m.splice(D, 1), H.state = OrgChart.EXPAND, r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
                ...
                if (H.children[b] == v)
                    if (-1 == H.collapsedChildrenIds.indexOf(v.id) && H.collapsedChildrenIds.push(v.id), H.children.splice(b, 1), "split" == v.templateName || "split" == H.templateName) {
                        if (M = v.parent) {
                            ...
                            for (var X = 0; X < M.childrenIds.length; X++) {
            ...
            for (b = H.children.length - 1; 0 <= b; b--) {
                if (OrgChart.server._mixedLayoutAllNodes(H))
                    if (0 == (z = H.children[b]).childrenIds.length && !z.isAssistant && "split" != z.templateName && (z.parent && "split" != z.parent.templateName || z.parent.isAssistant)) {
                        if ((z = H.children[b]).parent = null, z.layout = OrgChart.tree, H.children.splice(b, 1), H.childrenIds.splice(b, 1), t.layout == OrgChart.treeRightOffset && V.splice(0, 0, z.id), t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
                            var W;
                        ...
                        t.layout != OrgChart.treeRightOffset && V.splice(0, 0, z.id)
                    }
            }
    ...
    node: '<rect x="0" y="0" height="180" width="250" fill="none" stroke-width="1" stroke="#aeaeae"></rect>',
    ...
    nodeMenuButton: '<g style="cursor:pointer;" transform="matrix(1,0,0,1,225,105)" control-node-menu-id="{id}"><rect x="-4" y="-10" fill="#000000" fill-opacity="0" width="22" height="22"></rect><circle cx="0" cy="60" r="4" fill="#ffffff"></circle><circle cx="9" cy="60" r="4" fill="#ffffff"></circle><circle cx="18" cy="60" r="4" fill="#ffffff"></circle></g>',
    ...
    assistanseLink: '<path stroke-linejoin="round" stroke="#aeaeae" stroke-width="2px" fill="none" d="M{xa},{ya} {xb},{yb} {xc},{yc} {xd},{yd} L{xe},{ye}"/>',
    pointer: '<g data-pointer="pointer" transform="matrix(0,0,0,0,100,100)"><radialGradient id="pointerGradient"><stop stop-color="#ffffff" offset="0" /><stop stop-color="#C1C1C1" offset="1" /></radialGradient><circle cx="16" cy="16" r="16" stroke-width="1" stroke="#acacac" fill="url(#pointerGradient)"></circle></g>',
    node: '<rect x="0" y="0" height="180" width="250" fill="#039BE5" stroke-width="1" stroke="#aeaeae" rx="5" ry="5"></rect>',
    plus: '<circle cx="15" cy="15" r="15" fill="#ffffff" stroke="#aeaeae" stroke-width="1"></circle><line x1="4" y1="15" x2="26" y2="15" stroke-width="1" stroke="#aeaeae"></line><line x1="15" y1="4" x2="15" y2="26" stroke-width="1" stroke="#aeaeae"></line>',
    minus: '<circle cx="15" cy="15" r="15" fill="#ffffff" stroke="#aeaeae" stroke-width="1"></circle><line x1="4" y1="15" x2="26" y2="15" stroke-width="1" stroke="#aeaeae"></line>',
    nodeMenuButton: '<g style="cursor:pointer;" transform="matrix(1,0,0,1,225,105)" control-node-menu-id="{id}"><rect x="-4" y="-10" fill="#000000" fill-opacity="0" width="22" height="22"></rect><circle cx="0" cy="60" r="4" fill="#ffffff"></circle><circle cx="9" cy="60" r="4" fill="#ffffff"></circle><circle cx="18" cy="60" r="4" fill="#ffffff"></circle></g>',
    exportMenuButton: '<div style="position:absolute;right:{p}px;top:{p}px; width:40px;height:50px;cursor:pointer;" control-export-menu=""><hr style="background-color: #7A7A7A; height: 3px; border: none;"><hr style="background-color: #7A7A7A; height: 3px; border: none;"><hr style="background-color: #7A7A7A; height: 3px; border: none;"></div>',
    ...
    if ("multiline" == s) {
        ...
        u.setAttributeNS(null, "x", 140), u.setAttributeNS(null, "y", 70), u.setAttributeNS(null, "text-anchor", n), u.appendChild(C), d.appendChild(u);
        g = 1;
        var xx = 120;
        var yy = 70;
        o = 20;
        var xxMinus = 120; var yyPlus =70;
        ...
        if (null != d.innerHTML) {
            //var first = p[0].replace(",", " ");
            //var text = '<tspan x="140" y="50" text-anchor="end">* ' + first + ' </tspan>';
            //for (var m = 1; m < p.length; m++) {
            //    if (p[m] != "") {
            //        var tt = p[m].replace(",", " ");
            //        text = '<tspan x="140" y="50" text-anchor="end">* ' + tt + ' </tspan>'
            //    }
            //}
            v = d.innerHTML, d.innerHTML = "";
            //v = d.text, d.text = "";
        }
        else {
            var y = "";
    ...
    $(".field_0").attr("y", "20");
    $('[node-id="' + $("#VSCIdPointingEMP").val() + '"]').children().first().css("fill", "green");
