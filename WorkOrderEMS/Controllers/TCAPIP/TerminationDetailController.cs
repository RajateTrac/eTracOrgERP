    ...
    public class TerminationDetaillsController : Controller
    {
        ...
        public ActionResult SaveTerminationForm(TerminationModel Obj)
        {
            ...
            try
            {
                ...
                if (Obj != null)
                {
                    var TerminationSaved = _ITermination.SaveTerminationFormDetails(Obj, ObjLoginModel.UserId);
                    ...
                    if (TerminationSaved)
                    {
                        ...
                        return RedirectToAction("TerminationList");
                    }
                    else
                    {
                        return View("~/Views/TerminationDetail/TerminationForm.cshtml");
        ...
        public JsonResult EmployeeTerminationFinalizeList(DateTime? ToDate, DateTime? FromDate)
        {
            ...
            try
            {
                ...
                var getTList1 = _ITermination.TerminationlistFinalizedetails(ObjLoginModel.UserName, ToDate, FromDate);
        ...
        public JsonResult EmployeeTerminationPendingList(DateTime? ToDate, DateTime? FromDate)
        {
            ...
            try
            {
                ...
                var getTTerminationList = _ITermination.TerminationlistPendingdetails(ObjLoginModel.UserName, ToDate, FromDate);
        ...
        {
            List<string> LostAssets = new List<string>();
            int count = 1;
            ...
            try
            {
                ...
                if (isTrue == true)
                {
                    foreach (EmployeeAssetDetails item in obj)
                    {
                        count = count + 1;
                        if (item.AssetStatus == "L")
                        {
                            LostAssets.Add(item.AssetName);
                            if (count == objCount)
                            {
                                _ITermination.SendEmailToPayrollForLostAssets(LostAssets, EmpId, ObjLoginModel.UserName);
                            }
                        }
                        if (item.AssetStatus == "R")
                        {
                            if (count == objCount)
                            {
                                _ITermination.SendEmailToPayrollForAllAssetsReturned(obj, EmpId, ObjLoginModel.UserName);
                            }
                        }
                    }
        ...
        [HttpPost]
        public JsonResult PendingAssetStatusReturn(string EmployeeId, string AssetName)
        {
            try
            {
                _ITermination.SaveAssetStatusReturnDate(EmployeeId, AssetName);
            }
            catch (Exception ex)
            {
                ViewBag.Message = ex;
