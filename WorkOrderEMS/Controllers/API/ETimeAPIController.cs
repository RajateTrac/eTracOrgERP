...
namespace WorkOrderEMS.Controllers
{
    public class eTimeAPIController : ApiController
    {
        ...
        public JsonResult API_USPLeaveList(WorkOrderEMS.Models.Tbl_Employee_Leave_Management API)
        {
            int Result = 0;
            string Message = "";
            DataTable DT_Login = new DataTable();
            ...
            try
            {
                if (API.Id > 0)
                {
                    ...
                    dt = DBU.GetDTResponse(SQRY);
                    List<WorkOrderEMS.Models.Tbl_Employee_Leave_Management> Leave = DataRowToObject.CreateListFromTable<WorkOrderEMS.Models.Tbl_Employee_Leave_Management>(dt);
