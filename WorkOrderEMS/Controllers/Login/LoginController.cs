...
namespace WorkOrderEMS.Controllers.Login
{
    ...
    public class LoginController : Controller
    {
        ...
        [HttpPost]
        public ActionResult ChangePassword(eTracLoginModel objETracLoginModel)
        {
            bool status = false;
            //status
            string message = "";
            try
            {
                eTracLoginModel obj_eTracLoginModel = new eTracLoginModel();
                obj_eTracLoginModel = (eTracLoginModel)Session["eTrac"];
                objETracLoginModel.UserId = obj_eTracLoginModel.UserId;
                if (objETracLoginModel.UserId > 0 && !string.IsNullOrEmpty(objETracLoginModel.OldPassword) && !string.IsNullOrEmpty(objETracLoginModel.NewPassword))
                {
                    ...
                    if (status)
                    {
                        ...
                        FormsAuthentication.SignOut();
                        //FormsAuthentication.RedirectToLoginPage();
                        ViewBag.Message = CommonMessage.PasswordChanged();
                ...
                {
                    status = false;
                    message = CommonMessage.InvalidEntry();
                }
            }
            catch (Exception ex)
            { message = ex.Message; status = false; }
